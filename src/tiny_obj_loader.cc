  ...
  const vertex_index& i)
{
  ...
  if (i.vt_idx >= 0) {
	...
	// La siguiente linea ha sido modificada ya que las texturas se bindeaban al contrario
	// Se ha agregado el "1 -"
	texcoords.push_back(1 - in_texcoords[2*i.vt_idx+1]);
  }
  ...
  const char* mtl_basepath)
{
  ...
  while (ifs.peek() != -1) {
	...
	if ((0 == strncmp(token, "map_Ka", 6)) && isSpace(token[6])) {
	  token += 7;
	  material.ambient_texname = mtl_basepath + std::string(token);
	  continue;
	}
	...
	if ((0 == strncmp(token, "map_Kd", 6)) && isSpace(token[6])) {
	  token += 7;
	  material.diffuse_texname = mtl_basepath + std::string(token);
	  continue;
	}
	...
	if ((0 == strncmp(token, "map_Ks", 6)) && isSpace(token[6])) {
	  token += 7;
	  material.specular_texname = mtl_basepath + std::string(token);
	  continue;
	}
